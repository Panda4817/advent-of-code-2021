...
public class Range {
    ...
    public Optional<Range> getOverlapping(Range other) {
        Cube from = new Cube(Math.max(getFrom().getX(), other.getFrom().getX()), Math.max(getFrom().getY(), other.getFrom().getY()), Math.max(getFrom().getZ(), other.getFrom().getZ()));
        Cube to = new Cube(Math.min(getTo().getX(), other.getTo().getX()), Math.min(getTo().getY(), other.getTo().getY()), Math.min(getTo().getZ(), other.getTo().getZ()));
        boolean state = !getOn();
